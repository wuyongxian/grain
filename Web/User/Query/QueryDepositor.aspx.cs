...
{
    public partial class QueryDepositor : System.Web.UI.Page
    {
        ...
        private void DataBind(int pageindex,int pagesize)
        {
            ...
            if (QCunName.Value.Trim() == "" && QAccountNumber.Value.Trim() == "" && QName.Value.Trim() == "" && QIDCard.Value.Trim() == "")
            {
                string StrScript;
            ...
            strSql.Append("  SELECT top " + pagesize + " A.ID, B.AccountNumber,B.strName,C.strName AS CunID,CONVERT(varchar(100), A.StorageDate, 111) AS StorageDate,D.strName AS TypeID, E.strName AS TimeID,F.strName AS VarietyID,A.StorageNumber");
            ...
            {
                strSql.Append("   AND B.AccountNumber LIKE '%" + QAccountNumber.Value.Trim() + "%'");
            }
            if (QName.Value.Trim() != "")
            {
