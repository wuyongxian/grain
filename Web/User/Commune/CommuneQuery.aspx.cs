...
namespace Web.User.Commune
{
    public partial class CommuneQuery : System.Web.UI.Page
    {
        ...
        private void DataBind(int pageindex, int pagesize)
        {
            ...
            strSql.Append("   where 1=1  and A.numState=1");
            strSql.Append("   and B.ID="+WBID);
            strSql.Append("   and A.ID not in (select top "+pagesize*pageindex+" ID from Commune)");
            if (QWBID.Value.Trim() != "")
            {
        ...
        private void GetCount()
        {
            ...
            strSqlCount.Append("   where 1=1  and A.numState=1");
            strSqlCount.Append("   and B.ID=" + WBID);
            if (QWBID.Value.Trim() != "")
            {
        ...
        DataTable GetoutputTable()
        {
            ...
            string WBID = Session["WB_ID"].ToString();//当前网点ID
            StringBuilder strSql = new StringBuilder();
            strSql.Append("   SELECT  B.strName AS WBID,A.AccountNumber,A.strAddress,A.strName,A.IDCard,A.PhoneNO,A.FieldCopies,A.FieldCount,");
