...
{
    public partial class Settle_Shopping : System.Web.UI.Page
    {
        ...
        //    strSql.Append("  SELECT S.ID as SAID, A.ID,B.strName AS WBName,C.AccountNumber,C.strName AS DepName,'产品换购结算' AS BusinessName,CONVERT(NVARCHAR(100),A.dt_Sell,23) AS dt_Sell,DATEDIFF(DAY,A.dt_Sell,GETDATE()) AS dt_SellDay,");
        ...
        //    strSqlPara.Append("  FROM dbo.StorageShopping A INNER JOIN dbo.WB B ON A.WBID=B.ID");
        //    strSqlPara.Append("  INNER JOIN dbo.Depositor C ON A.Dep_AccountNumber=C.AccountNumber");
        //    strSqlPara.Append("  LEFT OUTER JOIN dbo.SA_Shopping S ON A.ID=S.StorageShoppingID");
        ...
        //    VarietyCount = 0; VarietyInterest = 0;  VarietyMoney = 0;
        ...
        //            if (dt.Rows[i]["ISReturn"].ToString() == "0")
        //            {
        //                JieSuan += Convert.ToDouble(dt.Rows[i]["VarietyMoney"]);
        //            }
        //            else
        //            {
        //                JieSuan -= Convert.ToDouble(dt.Rows[i]["VarietyMoney"]);
        ...
        private void BindData(string WBID, string dtStart, string dtEnd)
        {
            ...
            strSql.Append("  LEFT OUTER JOIN dbo.SA_Shopping S ON A.ID=S.StorageShoppingID");
            strSql.Append(" order by A.dt_Sell desc  ");
            DataTable dt = SQLHelper.ExecuteDataTable(strSql.ToString());
            Repeater1.DataSource = dt;
            Repeater1.DataBind();
        ...
        public string GetVarietyMoney(object VarietyMoney, object ISReturn)
        {
            if (Convert.ToInt32(ISReturn) == 0)//第一次存转销的记录
            {
                return "<span style='color:Green'>" + VarietyMoney + "</span>";
            }
            else
            {
                return "<span style='color:Red'>-" + VarietyMoney + "</span>";
            }
        }
        ...
        public string GetOperateInfo(object ID)
        {
            string strReturn = "";
            //查询当前的这笔记录是否已经有结算的记录
            object objID = SQLHelper.ExecuteScalar("SELECT ID FROM dbo.SA_Shopping WHERE StorageShoppingID=" + ID);
